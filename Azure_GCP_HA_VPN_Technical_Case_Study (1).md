...
High availability and backup address different failure conditions. Redundant VPN paths can maintain connectivity during an infrastructure failure, but they do not protect against data loss, corruption, accidental deletion, or destructive administrative changes.

For this scenario, I would define backup and recovery requirements according to the component and its business role. Critical application data, configuration, security policy, and other recoverable state should have appropriate backup or versioning protection based on the institution's recovery objectives and retention requirements.

The asynchronous messaging layer provides temporary durable retention when the GCP consumer is unavailable, but it should not be treated as a substitute for backup. Likewise, infrastructure configuration should be recoverable to a known-good state through approved configuration backups, version-controlled Infrastructure as Code where adopted, or other documented recovery mechanisms.

Recovery procedures should be tested periodically to verify that protected data and configuration can actually be restored within the required recovery objectives.
